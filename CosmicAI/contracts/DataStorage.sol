...
contract DataStorage is Ownable {
    ...
    mapping(uint256 => AstronomicalData) private dataRecords;
    ...
    // Function to check if data is analyzed
    function isDataAnalyzed(uint256 _id) external view returns (bool) {
        return dataRecords[_id].analyzed;
    }

    // Function to get the analysis result of a specific data record
    function getAnalysisResult(uint256 _id) external view returns (string memory) {
        require(dataRecords[_id].analyzed, "Data has not been analyzed yet");
        return dataRecords[_id].analysisResult;
    }
